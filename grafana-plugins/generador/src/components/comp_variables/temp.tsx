import React from 'react'

type data = {
  temp: number
}

const temp = ({temp}:data) =>{
    return(
        <g id="g577" transform="translate(2.472 2.12)">
          <text
            id="uma1_ing_sonda_0"
            x="205.018"
            y="48.038"
            fill="#f2f2f2"
            strokeWidth="0.289"
            fontSize="11.289"
            transform="scale(1.0508 .95161)"
            style={{ lineHeight: "1.25" }}
            fontFamily="sans-serif"
            letterSpacing="0"
            wordSpacing="0"
            xmlSpace="preserve"
          >
            <tspan
              id="tspan1960-4"
              x="205.018"
              y="48.038"
              fill="#f2f2f2"
              strokeWidth="0.289"
              dy="0"
              fontFamily="BankGothic Lt BT"
              fontSize="11.289"
              fontWeight="bold"
            >
              {temp}
            </tspan>
          </text>
          <text
            id="text21805"
            x="194.035"
            y="21.416"
            style={{ lineHeight: "1.25" }}
            fill="#f9f9f9"
            strokeWidth="0.265"
            fontFamily="sans-serif"
            fontSize="3.528"
            letterSpacing="0"
            wordSpacing="0"
            xmlSpace="preserve"
          >
            <tspan
              id="tspan21803"
              x="194.035"
              y="21.416"
              fill="#f9f9f9"
              strokeWidth="0.265"
              fontFamily="BankGothic Lt BT"
              fontSize="4.586"
            >
              <tspan id="tspan3013" fontSize="5.644">
                TEMPERATURA
              </tspan>
            </tspan>
          </text>
          <g id="g2977" transform="matrix(1.0566 0 0 1.0053 -15.093 -3.151)">
            <path
              id="path14959"
              fill="url(#linearGradient566)"
              d="M251.6 32.723l.115-4.921-9.994-9.952-5.055-.018z"
            ></path>
            <path
              id="path14961"
              fill="url(#linearGradient566)"
              d="M198.01 26.725l-.325 30.288 2.22-2.041v-26.76z"
            ></path>
            <path
              id="path884-8-0"
              fill="#fc0"
              fillRule="evenodd"
              d="M386.1 38.143a22.249 22.248 0 01.495.026l-.025 4.723a17.518 17.517 0 00-.47-.018 17.518 17.517 0 00-17.519 17.517 17.518 17.517 0 006.337 13.466l-2.697 3.897a22.249 22.248 0 01-8.372-17.362 22.249 22.248 0 0122.25-22.248zm1.66.087a22.249 22.248 0 0112.97 5.444l-3.745 3.013a17.518 17.517 0 00-9.25-3.719zm13.831 6.248a22.249 22.248 0 016.125 10.751l-4.823.208a17.518 17.517 0 00-5.012-7.972zm6.385 11.906a22.249 22.248 0 01.374 4.007 22.249 22.248 0 01-1.587 8.152l-4.246-2.105a17.518 17.517 0 001.101-6.047 17.518 17.517 0 00-.43-3.8zm-5.899 11.137l4.226 2.094a22.249 22.248 0 01-7.293 8.852l-2.52-4a17.518 17.517 0 005.587-6.946zm-26.232 7.045a17.518 17.517 0 009.558 3.315l-.212 4.71a22.249 22.248 0 01-12.04-4.133zm19.675.55l2.536 4.025a22.249 22.248 0 01-11.699 3.488l.214-4.746a17.518 17.517 0 008.95-2.767z"
              clipPath="url(#clipPath1984)"
              opacity="0.75"
              paintOrder="markers stroke fill"
              transform="matrix(.77865 0 0 .68008 -71.499 10.622)"
            ></path>
            <g
              id="g1170-0"
              strokeWidth="1.049"
              opacity="0.75"
              transform="matrix(-1.2345 0 0 1.0781 329.3 -31.086)"
            >
              <path
                id="path829-0"
                fill="none"
                stroke="#04e6f4"
                strokeWidth="0.278"
                d="M55.846 66.713L74.839 45.83l30.144-.094 4.158 4.063v1.89l-4.063 3.59v24.663l1.7 1.89v2.363l-6.236 6.709v3.59l2.457 2.552v4.158h-14.93l-2.93-2.457-5.102 5.197h-18.71l-5.198-5.386z"
              ></path>
              <path
                id="path831-8"
                fill="#0cedf7"
                d="M62.555 59.437l-.094-4.536 8.22-9.171 4.159-.017z"
              ></path>
              <path
                id="path833-1"
                fill="#0cedf7"
                d="M106.64 53.909l.267 27.915-1.826-1.881V55.278z"
              ></path>
            </g>
            <g
              id="g6103"
              fill="#168498"
              stroke="#0deff7"
              transform="matrix(1.1276 0 0 1.0876 -35.928 -30.814)"
            >
              <path
                id="path852-9"
                strokeWidth="0.618"
                d="M260.44 88.374l-12.621 12.459"
              ></path>
              <path
                id="path852-0-1"
                strokeWidth="0.289"
                d="M255.17 89.828l-6.212 5.99"
              ></path>
              <path
                id="path852-0-7-2"
                strokeWidth="0.289"
                d="M257.86 94.466l-4.455 4.302"
              ></path>
            </g>
            <path
              id="path2051"
              fill="#168498"
              fillRule="evenodd"
              d="M386.21 36.256a23.653 23.651 0 00-23.653 23.651 23.653 23.651 0 0023.653 23.651 23.653 23.651 0 0023.652-23.651 23.653 23.651 0 00-23.652-23.651zm0 3.546a20.179 20.106 0 0120.178 20.105 20.179 20.106 0 01-20.178 20.106 20.179 20.106 0 01-20.179-20.106 20.179 20.106 0 0120.179-20.105z"
              clipPath="url(#clipPath2072)"
              opacity="0.75"
              paintOrder="markers stroke fill"
              transform="matrix(.99011 0 0 .86477 -153.5 .047)"
            ></path>
            <path
              id="rect2077"
              fill="#10677d"
              fillRule="evenodd"
              d="M203.48 57.053H211.816V60.519999999999996H203.48z"
              opacity="0.75"
              paintOrder="markers stroke fill"
            ></path>
            <path
              id="rect2077-1"
              fill="#10677d"
              fillRule="evenodd"
              d="M203.48 52.358H211.816V55.824999999999996H203.48z"
              opacity="0.75"
              paintOrder="markers stroke fill"
            ></path>
            <path
              id="rect2077-7"
              fill="#10677d"
              fillRule="evenodd"
              d="M203.48 47.663H211.816V51.129999999999995H203.48z"
              opacity="0.75"
              paintOrder="markers stroke fill"
            ></path>
            <path
              id="rect2077-1-2"
              fill="#10677d"
              fillRule="evenodd"
              d="M203.48 42.968H211.816V46.435H203.48z"
              opacity="0.75"
              paintOrder="markers stroke fill"
            ></path>
            <path
              id="rect2077-9"
              fill="#10677d"
              fillRule="evenodd"
              d="M203.48 38.274H211.816V41.741H203.48z"
              opacity="0.75"
              paintOrder="markers stroke fill"
            ></path>
            <path
              id="rect2077-1-5"
              fill="#10677d"
              fillRule="evenodd"
              d="M203.48 33.579H211.816V37.046H203.48z"
              opacity="0.75"
              paintOrder="markers stroke fill"
            ></path>
            <path
              id="path15126"
              fill="none"
              stroke="#fcfcfc"
              strokeDasharray="0.80648, 0.80648"
              strokeWidth="0.269"
              d="M235.71 32.103l2.423-1.664-5.885-3.979h-32.654l-4.269-3.69"
              opacity="0.75"
            ></path>
          </g>
          <text
            id="uma1_ing_sonda_0-6"
            x="212.125"
            y="59.059"
            fill="#f2f2f2"
            strokeWidth="0.289"
            fontSize="5.895"
            transform="scale(1.0508 .95161)"
            style={{ lineHeight: "1.25" }}
            fontFamily="sans-serif"
            letterSpacing="0"
            wordSpacing="0"
            xmlSpace="preserve"
          >
            <tspan
              id="tspan1960-4-8"
              x="212.125"
              y="59.059"
              fill="#f2f2f2"
              strokeWidth="0.289"
              fontFamily="BankGothic Lt BT"
              fontSize="9.878"
              fontWeight="bold"
            >
              °C
            </tspan>
          </text>
        </g>
        
    )
}

export default temp